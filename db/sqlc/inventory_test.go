...
func createRandomInventory(t *testing.T) Inventory {
	...
	require.Equal(t, arg.CompanyID, inventory.CompanyID)
	require.Equal(t, arg.ProductID, inventory.ProductID)
	require.Equal(t, arg.AmountAvailable, inventory.AmountAvailable)
...
func TestUpdateInventory(t *testing.T) {
	...
	inventory2, err := testQueries.UpdateInventory(context.Background(), arg)
	require.NoError(t, err)
	require.NotEmpty(t, inventory2)

	require.Equal(t, inventory1.ID, inventory2.ID)
	require.Equal(t, arg.CompanyID, inventory2.CompanyID)
	require.Equal(t, arg.ProductID, inventory2.ProductID)
	require.Equal(t, arg.AmountAvailable, inventory2.AmountAvailable)
	require.WithinDuration(t, inventory1.CreatedAt, inventory2.CreatedAt, time.Second)
}
...
func TestGetCompanyProductInventory(t *testing.T) {
	...
	arg := GetCompanyProductInventoryParams{
		CompanyID: inventory1.CompanyID,
		ProductID: inventory1.ProductID,
	}

	inventory2, err := testQueries.GetCompanyProductInventory(context.Background(), arg)
	require.NoError(t, err)
	require.NotEmpty(t, inventory2)
